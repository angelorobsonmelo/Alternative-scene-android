...
interface UserApiDataSource {
    ...
    fun findByEmailAndGoogleAccountId(@Query("email") email: String, @Query("googleAccountId") googleAccountId: String): Observable<ResponseBase<AuthResponse>>

    @POST("v1/users_app")
    fun save(@Body userRequest: UserRequest): Observable<ResponseBase<AuthResponse>>
}